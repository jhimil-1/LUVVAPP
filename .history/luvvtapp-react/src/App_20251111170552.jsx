...
import { Heart, Send, User, Users, MessageCircle, Plus, Settings, LogOut, Sparkles, Menu, X, Edit2, Trash2 } from 'lucide-react';
...
const App = () => {
  ...
  const [assessmentPersonality, setAssessmentPersonality] = useState('');
  const [assessmentLove, setAssessmentLove] = useState('');
  ...
  const openAssessment = () => {
    setAssessmentPersonality(userProfile?.self_assessment?.personality_type || '');
    setAssessmentLove(userProfile?.self_assessment?.love_language || '');
    setAssessmentValues(userProfile?.self_assessment?.relationship_values || []);
    setAssessmentError('');
    setAdviceOpen(false);
  ...
  const saveAssessment = async () => {
    if (!userId) {
      setAssessmentError('Please login first.');
      return;
    }
    setAssessmentSaving(true);
    setAssessmentError('');
    const assessment = {};
    if (assessmentPersonality) assessment.personality_type = assessmentPersonality;
    if (assessmentLove) assessment.love_language = assessmentLove;
    if (assessmentValues && assessmentValues.length) assessment.relationship_values = assessmentValues;
    try {
      const res = await fetch(`${API_BASE_URL}/api/users/${userId}/assessment`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        setAssessmentError(err?.detail || 'Failed to save assessment.');
      }
    } catch (e) {
      setAssessmentError('Network error while saving.');
    } finally {
  ...
  const loadUserProfile = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Stale or invalid user_id in local storage -> clear and require auth again
        localStorage.removeItem('luvvtapp_user_id');
        setUserId(null);
        setUserProfile(null);
      }
    } catch (error) {
  ...
  const saveEditRelationship = async () => {
    if (!editingRelationship || !userId) {
      console.error('Cannot save: editingRelationship or userId is missing', { editingRelationship, userId });
      return;
    }
    
    console.log('Saving relationship:', editingRelationship);
  ...
  const createUser = async (name, email, assessment) => {
    // Legacy login-or-create fallback
    try {
      const response = await fetch(`${API_BASE_URL}/api/users`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, email, self_assessment: assessment })
      });
      if (response.ok) {
        const data = await response.json();
        const returnedUserId = data?.user_id;
        if (returnedUserId) {
          setUserId(returnedUserId);
          localStorage.setItem('luvvtapp_user_id', returnedUserId);
          setUserProfile(data.profile || null);
          await Promise.all([loadUserProfile(), loadRelationships(), loadSessions(), loadAdviceList()]);
        }
      }
    } catch (error) {
  ...
  const signup = async () => {
    ...
    try {
      ...
      if (uid) {
        localStorage.setItem('luvvtapp_user_id', uid);
        setUserId(uid);
        setUserProfile(data.profile || null);
        await Promise.all([loadUserProfile(), loadRelationships(), loadSessions(), loadAdviceList()]);
      }
  ...
  if (!userId || !userProfile) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-50 via-purple-50 to-blue-50 p-4">
        <div className="w-full max-w-md bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-pink-100 p-6">
          <div className="flex items-center gap-2 mb-4">
            <Heart className="w-6 h-6 text-pink-500 fill-pink-500" />
            <h2 className="text-xl font-semibold">LuvvTapp</h2>
          </div>
          <div className="flex gap-2 mb-4">
            <button
              className={`flex-1 py-2 rounded-lg ${authMode==='signup' ? 'bg-pink-500 text-white' : 'bg-pink-50 text-pink-700'}`}
              onClick={() => setAuthMode('signup')}
            >Signup</button>
            <button
              className={`flex-1 py-2 rounded-lg ${authMode==='login' ? 'bg-purple-600 text-white' : 'bg-purple-50 text-purple-700'}`}
          ...
          {authMode === 'signup' && (
            <>
              ...
              <input value={nameInput} onChange={e=>setNameInput(e.target.value)} className="w-full mb-3 p-2 border rounded-lg" placeholder="Your name" />
            </>
          )}

          <label className="block text-sm text-gray-700 mb-1">Email</label>
          <input value={emailInput} onChange={e=>setEmailInput(e.target.value)} className="w-full mb-3 p-2 border rounded-lg" placeholder="you@example.com" />

          <label className="block text-sm text-gray-700 mb-1">Password</label>
          <input type="password" value={passwordInput} onChange={e=>setPasswordInput(e.target.value)} className="w-full mb-4 p-2 border rounded-lg" placeholder="••••••••" />

          <button
            onClick={authMode==='signup' ? signup : login}
            className="w-full py-2 rounded-lg bg-gradient-to-r from-pink-500 to-purple-600 text-white font-medium"
          >{authMode==='signup' ? 'Create account' : 'Login'}</button>

          <div className="text-xs text-gray-500 mt-3">By continuing you agree to our friendly terms.</div>
        </div>
      </div>
    );
  }
        ...
        <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 fixed lg:sticky top-[73px] left-0 h-[calc(100vh-73px)] w-80 bg-white/80 backdrop-blur-md border-r border-pink-100 p-6 transition-transform duration-300 z-40 overflow-y-auto`}>
          ...
          <div className="space-y-2">
            ...
            {[
              ...
              { icon: Sparkles, text: 'Self Assessment', action: openAssessment },
        ...
        <main className="flex-1 h-[calc(100vh-73px)] flex flex-col">
          {currentView === 'chat' ? (
            <>
              ...
              {assessmentOpen && (
                <div ref={assessmentRef} className="p-6 bg-gradient-to-br from-pink-50 to-purple-50 border-b">
                  <div className="max-w-2xl mx-auto">
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-xl font-bold text-gray-800">Self Assessment</h3>
                      <button 
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <select
                          value={assessmentPersonality}
                          onChange={(e) => setAssessmentPersonality(e.target.value)}
                          className="w-full px-4 py-3 rounded-xl border-2 border-pink-200 focus:border-pink-400 focus:outline-none"
                        >
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Love Language</label>
                        <select
                          value={assessmentLove}
                          onChange={(e) => setAssessmentLove(e.target.value)}
                          className="w-full px-4 py-3 rounded-xl border-2 border-pink-200 focus:border-pink-400 focus:outline-none"
                        >
                          <option value="">Select one (optional)</option>
                          {['Words of Affirmation', 'Quality Time', 'Receiving Gifts', 'Acts of Service', 'Physical Touch'].map(lang => (
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">What matters most in relationships? (comma-separated)</label>
                        <textarea
                          value={assessmentValues.join(', ')}
                          onChange={(e) => setAssessmentValues(e.target.value.split(',').map(v => v.trim()).filter(v => v))}
                          className="w-full px-4 py-3 rounded-xl border-2 border-pink-200 focus:border-pink-400 focus:outline-none"
                          placeholder="e.g., honesty, communication, trust, shared values"
                          rows={3}
                        />
                      </div>
                      
                      {assessmentError && (
                        <div className="text-red-600 text-sm">{assessmentError}</div>
                      )}
                      ...
                      <div className="flex gap-3">
                        <button
                          ...
                          className="px-6 py-3 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-xl font-medium hover:shadow-lg transition-all disabled:opacity-50"
                        >
                          {assessmentSaving ? 'Saving...' : 'Save Assessment'}
                        </button>
                        <button
                          onClick={() => setAssessmentOpen(false)}
                          className="px-6 py-3 border-2 border-pink-300 text-pink-600 rounded-xl font-medium hover:bg-pink-50 transition-all"
                        >
                          Cancel
                        </button>
                      </div>
